...
// GetPoliciesByUserIDAdmin handles fetching a specific client's full Health Insurance list for the
...
// @Tags         Health Insurance (Admin)
// @Accept       json
// @Produce      json
// @Param        userId  path      string  true  "Target Client User ID"
// @Success      200     {object}  response.APIResponse{data=domain.HealthInsuranceListResponse}  "Policies retrieved successfully"
// @Failure      401     {object}  response.APIResponse  "Unauthorized"
// @Failure      403     {object}  response.APIResponse  "Forbidden — Admin role required"
// @Security     BearerAuth
// @Router       /health-insurances/user/{userId} [get]
func (h *HealthInsuranceHandler) GetPoliciesByUserIDAdmin(c *gin.Context) {
	claims, ok := middleware.GetClaims(c)
	if !ok {
		response.Error(c, http.StatusUnauthorized, "unauthorized")
		return
...
// @Produce      json
// @Param        id   path      string  true  "Policy ID"
// @Success      200  {object}  response.APIResponse{data=domain.HealthInsurance}
// @Failure      400  {object}  response.APIResponse
// @Security     BearerAuth
// @Router       /health-insurances/{id}/mark-paid [post]
func (h *HealthInsuranceHandler) MarkPremiumPaid(c *gin.Context) {
	claims, ok := middleware.GetClaims(c)
	if !ok {
		response.Error(c, http.StatusUnauthorized, "unauthorized")
		return
